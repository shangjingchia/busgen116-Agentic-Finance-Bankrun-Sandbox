# AGENTS.md — Agentic Bank Run Sandbox: Operating Context

This file is loaded by Codex at the start of every session. Read it before starting any task. If a request seems to conflict with anything here, raise it before proceeding.
...
- **LLM API:** Anthropic Python SDK. Default to `Codex-haiku-4-5-20251001` for routine agent decisions. Use `Codex-sonnet-4-6` for strategic moments (initial portfolio assessment, run-or-stay decision under high stress) where reasoning quality matters more than cost. Both models accessed via the Messages API.
